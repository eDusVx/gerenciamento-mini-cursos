...
class EfetuarLoginUsuarioUsecase:
    def __init__(self,usuario_repository:UserRepositoryInteface,authService:AuthServiceInterface):
    ...
    async def execute(self, email:str, senha:str) -> str:
        try:
            ...
            if validarSenha == True:
                token = self.authService.autenticar(usuario.nome, usuario.email, usuario.cpf, usuario.tipoAcesso)
                return token
            else:
